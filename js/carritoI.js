function mostrarCarrito() 
{
    ...
    const tarjetita = document.getElementById('videojuegos-carrito');
    ...
    if (carrito.length === 0) {
      tarjetita.innerHTML = '<p>No hay productos en el carrito</p>';
      return;
    }
    ...
    for (let e = 0; e < carrito.length; e++) {
      const producto = carrito[e];
      totalPrecio += producto.precioTotal;
      tarjetita.innerHTML += `
        <div>
        <img src="${producto.imagenJue}" alt="" class="tamanio">
          <p>Nombre: ${producto.nombre}</p>
          <p>Precio unitario: ${producto.precioJue}</p>
          <p>Cantidad: <input type="number" min="1" value="${producto.cantidad}" oninput="cambiarCantidad(${e}, this.value)"></p>
          <p>Precio total: $${producto.precioTotal.toFixed(2)}</p>
          <button id="eliminar" onclick="eliminarProducto(${e})">Eliminar</button>
        </div>
      `;
...
`;
  ...
  function generarFactura(nombre, direccion, ciudad, telefono, carrito) {
    ...
    carrito.forEach((producto) => {
      videojuegos += `${producto.nombre} x ${producto.cantidad}: $${producto.precioTotal.toFixed(2)}\n`;
