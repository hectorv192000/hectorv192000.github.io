            ...
            `<div class="tarjetita">
              <img  src="${videojuegos[e].imagenJue}" onclick="mostrarModal(${e})">
                <div class="informacion">
                  ...
                  <button class="boted1" onclick=comprar(${e})>Comprar</button>
                </div>
            </div>`
        }
...
function comprar(indice) 
{
  ...
  for (let e = 0; e < lista.length; e++) {
    if (lista[e].nombre === videojuegos[indice].nombre) {
      lista[e].cantidad += 1;
  ...
  if (!productoExistente) {
    const producto = {
      nombre: videojuegos[indice].nombreJue,
  ...
  let e = 0
  while (llevar == true) {
    if (videojuegos[e].nombre == videojuegos[indice].nombre) {
      videojuegos[e].existencia -= 1
      if (videojuegos[e].existencia == 0) {
        verVentanitas()
      }
      llevar = false
...
function mostrarElemtrosLista() {
  ...
  for (let e = 0; e < lista.length; e++) {
      ...
      productosCompra.innerHTML += `
      <div><div class="img">
      <button onclick=eliminar(${e}) class="botonTrash">
      <img src="./img/trash.png">
      <img src="${videojuegos[e].imagenJue}">
      </button><p>${producto.nombre} (x${producto.cantidad})</p>
      </div><p></p> $${precioTotal.toFixed(2)}</p></div>`;
  }
